...
public class SuspendedBtnController {
  ...
  public void initialize() {
    ...
    delete_btn.setOnAction(event -> handleDeleteAction());
    loadConfig();
  }
  ...
  private void handleDeleteAction() {
    if (username != null) {
      try {
        HttpClient client = HttpClient.newHttpClient();
        HttpRequest request = HttpRequest.newBuilder()
            .uri(URI.create("http://" + url + ":" + port + "8080/api/users/delete?username=" + username))
            .POST(HttpRequest.BodyPublishers.noBody())
            .build();
